...
def init_splats(num_points, bbox_min, bbox_max, device, mean_color, pad_xy, pad_z):
    ...
    # SH DC convention (3DGS paper) — store coeff so that colored =
    #   SH(0) * f_dc + 0.5, with SH(0) = 0.28209... .  Here we parameterize
    #   just as logit colors and apply sigmoid at render time.
...
def ssim_loss(img1, img2):
    """Simple SSIM on RGB, windowed. img: (H, W, 3) in [0,1]."""
    # 11x11 Gaussian kernel (precomputed approximation)
    k = torch.tensor(
        [
            0.000814,
            0.005981,
    ...
    # Use mean-of-squared-diffs as a lightweight proxy instead of full SSIM
    # (to avoid a big kernel rewrite; L1 carries most of the signal anyway)
...
def main():
    ...
    for it in range(args.iters):
        ...
        loss = torch.nn.functional.l1_loss(render_rgb, gt)
        ...
        if (it + 1) % 200 == 0:
            psnr = -20 * math.log10(ema_loss + 1e-9)  # approx PSNR from L1
            N = splats["means"].shape[0]
            elapsed = time.time() - t0
            line = (f"iter={it + 1:6d}/{args.iters} loss={loss_v:.5f} "
                    f"ema={ema_loss:.5f} psnr~={psnr:.1f} N={N} {elapsed:.0f}s")
            print(line)
            log_f.write(line + "\n")
            log_f.flush()
    ...
    print(f"[done] {time.time() - t0:.0f}s, peak VRAM={peak_mem:.2f} GB, final N={N_final}")
