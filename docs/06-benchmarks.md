...
| SchurVINS | 🟡 2026-04-23 晚 mono-IMU 15 poses/336s wall, 位置跳 (743m, 243m, 103m). 2026-04-27 重诊:disparity threshold fix(`init_min_disparity 30→10.0`,**必须 FLOAT**,YAML int 静默用默认 40 — vk::param<double> 不接受 int)+ `init_min_features 60→30` 让 frontend 真正 init,但暴露 deeper bug — frontend 在弱纹理 fisheye 上反复 RELOCALIZE→RESET,backend 命中 fatal `CHECK failed: old_rows ==... ` 在 schur_vins.cpp:112,过程 abort. 比之前进一步,但需要 frontend feature 增强 + backend reset 状态修复才能跑通 | ⬜ | 🔴 2026-04-23 晚 build 全通 (Ceres 2.2 Manifold port + 循环依赖断 + dt assert 放宽 + IMU handler assert 放宽 + resetBackend 加 SchurVINS ResetStateForReinit). 但 runtime 一直不收敛:mono-IMU **SE3 8.5m / Sim3 1.7m** (81 poses / 130s, 81/2880 frames = 2.8% tracking 成功率, filter 每 ~1-2s re-init 一次). stereo-IMU **SE3 20km / Sim3 1.4m** (2593 poses 但 pose 从 174m 开始单调飞到 km 级, 应该是 stereo 三角化出错或 IMU bias 初始化错). 3 次迭代都没能稳定. 结论: **port 本身没问题** (22 pkgs clean build, symbols linked, 运行不崩), 但 SchurVINS 自己的 frontend+backend 耦合对 V101 不鲁棒 —— 不像 ORB_SLAM3/AirSLAM 的 0.08m 水平. 论文的 0.1m 结果可能需要 stereo + 特定的 init sequence + 本仓库未暴露的 param, 本 ws 达不到 paper 复现 | ⬜ |
