...
| mins | ⬜ (ROS1) | ⬜ (ROS1) | ⬜ | — |
| EPLF-VINS | 🟡 2026-04-27 mono-IMU 仅初始化 33s 段(63 poses), 后 "Not enough features or parallax" 停发. **APE Sim3 0.318m / SE3 2.67m**(8× scale off, IMU bias init 错). feature_tracker 1280×1024 fisheye CPU LK 256ms/帧饱和;line_tracker 落后 175s. max_cnt 300→100 / equalize off / bag 0.5× 都救不了. 同 VINS-Fusion init failure / SchurVINS 15 poses / open_vins 1024m 一样的 FB100+鱼眼+mono-IMU 病. 锁 stereo-IMU | — | 🟢 2026-04-27 V101 mono-inertial **APE SE3 0.066m / Sim3 0.058m** (1401 poses, 140s 完整 bag, paper-class). 同档 ORB_SLAM3 stereo 0.082m,优于 VINS-Fusion stereo+IMU 0.132m。完整 ROS 1 catkin → ROS 2 ament + Ceres 2.2 Manifold port: 3 包build clean (lf_feature_tracker / linefeature_tracker / lfvins_estimator),5 包 COLCON_IGNORE。 模式: ros1_compat.h shim + 复用 VINS-Fusion-ROS2 的 PoseLocalParameterization Manifold | — |
