...
|---------------|--------------------------|-----------------------------|------------|-----------|
| open_vins | 🟢 **2026-04-27 mono-inertial 全 bag 跑通, 1.0× rate APE Sim3 2.10m / SE3 2.94m** (0.5× rate 类似 Sim3 2.44m / SE3 3.28m). 307k poses, 418s 完整 bag,首个完成 GeoScan B1 mono-IMU 全 bag 的 VIO. 关键修复:`init_max_disparity: 0.30 → 3.0` (fisheye+LK 运动 disparity 才 ~5-10px,默认 0.30 让 static init 永不触发 → fallback 到 dyn init → ba bias 估错 → 飞). 之前的 "FB100 zero-DT" 诊断**已纠正**(bag 实测 0 zero-DT). 见 `memory/project_openvins_geoscan_divergence.md` | 🟢 2026-04-16 stereo-IMU APE 0.76m vs finder (SE3),轨迹 `~/Documents/Datasets/geoscan/B1/2026-02-12-16-47-48/open-vins.txt` | 🟢 2026-04-22 V1_01 stereo-IMU **APE SE3 0.165m / Sim3 0.159m** (2780 poses, bag replay). workspace 重整 + `init_dyn_use: true` 后救活（原 static init 等不到 accel jerk → 94m scale drift）。另保留 4-16 ASL pub 试点 0.604m/first 2000 poses | — |
...
| mins | ⬜ (ROS1) | ⬜ (ROS1) | ⬜ | — |
| EPLF-VINS | 🟢 **2026-04-27 v3 全 bag 跑通!**`image_downsample_2x.py` republisher 把 1280×1024→640×512 + intrinsics÷2 + max_cnt=100 + equalize=0 解了 LK CPU 饱和。**APE Sim3 4.03m / SE3 5.47m** (2253 poses, 382s 完整 bag, 1.0× rate). 第二个完成 GeoScan mono-IMU 全 bag 的 VIO(open_vins 2.10m 第一). v1 的 "33s 后停发" 完全是 LK CPU 饱和导致的,跟算法层无关. | — | 🟢 2026-04-27 V101 mono-inertial **APE SE3 0.066m / Sim3 0.058m** (1401 poses, 140s 完整 bag, paper-class). 同档 ORB_SLAM3 stereo 0.082m,优于 VINS-Fusion stereo+IMU 0.132m。完整 ROS 1 catkin → ROS 2 ament + Ceres 2.2 Manifold port: 3 包build clean (lf_feature_tracker / linefeature_tracker / lfvins_estimator),5 包 COLCON_IGNORE。 模式: ros1_compat.h shim + 复用 VINS-Fusion-ROS2 的 PoseLocalParameterization Manifold | — |
...
| SchurVINS | 🔴 2026-04-23 晚 mono-IMU 15 poses/336s wall. 2026-04-27 三次重诊:(a) disparity threshold fix(必须 FLOAT)+ init_min_features 30 让 frontend 真正 init;(b) v5 disparity fix → backend `Check failed: old_rows==(15+states_map*6-6)` 在 schur_vins.cpp:112 abort;(c) v6 + image downsample 2× → 同 backend Check failed abort. 失败在 backend RELOCALIZE→RESET 状态机不变量,不是 frontend LK 瓶颈 — disparity / downsample 都救不了,需要 schur_vins backend 源码 patch | ⬜ | 🔴 2026-04-23 晚 build 全通 (Ceres 2.2 Manifold port + 循环依赖断 + dt assert 放宽 + IMU handler assert 放宽 + resetBackend 加 SchurVINS ResetStateForReinit). 但 runtime 一直不收敛:mono-IMU **SE3 8.5m / Sim3 1.7m** (81 poses / 130s, 81/2880 frames = 2.8% tracking 成功率, filter 每 ~1-2s re-init 一次). stereo-IMU **SE3 20km / Sim3 1.4m** (2593 poses 但 pose 从 174m 开始单调飞到 km 级, 应该是 stereo 三角化出错或 IMU bias 初始化错). 3 次迭代都没能稳定. 结论: **port 本身没问题** (22 pkgs clean build, symbols linked, 运行不崩), 但 SchurVINS 自己的 frontend+backend 耦合对 V101 不鲁棒 —— 不像 ORB_SLAM3/AirSLAM 的 0.08m 水平. 论文的 0.1m 结果可能需要 stereo + 特定的 init sequence + 本仓库未暴露的 param, 本 ws 达不到 paper 复现 | ⬜ |
| ORB_SLAM3_ROS2 | 🟢（参考基线） | 🟢 | 🟢 2026-04-22 V1_01 stereo-inertial **APE SE3 0.082m / Sim3 0.076m** (1770 poses). 复用 `~/casbot_ws/.../glim_ext/.../ORB_SLAM3/stereo_inertial_euroc` aarch64 预编译 binary,绕开 ORB_SLAM3_ROS2 wrapper 的 x86_64 `libORB_SLAM3.so`. **2026-04-27 mono-inertial 也跑了:APE SE3 0.023m / Sim3 0.012m** (2799 poses, 比 stereo 还好 3.5×). 注意需重生成 EuRoC_TimeStamps/V101.txt 匹配实际 `...000.png` 文件名(原文件期待 `...976` ns 精度,与提取版冲突) | ⬜ |
...
| DPVO | 🟢 2026-04-28 Spark port + 全 bag(4183f, stride=1, ~17min): **APE Sim3 6.21m / SE3 21.08m**(纯单目无 IMU,scale 漂大). 老 4-17 数字 0.60m 是仅前 1000f 段(101s),drift 没累积. mono-only 在长 bag 上输给 mono-IMU VIO(open_vins 2.10m,EPLF 4.03m)。Spark port 需 4 个 patch:cpp_extension CUDA 检查、torch_scatter CUDA 检查、dpvo 里 `.type()`→`.scalar_type()` | — | 🟢 2026-04-20 V1_01 APE 0.046m (1435 pairs, stride=2) | — |
